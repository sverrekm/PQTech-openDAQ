...
export default function UpdateCard() {
  ...
  const [restartFase, setRestartFase] = useState<'bruker' | 'venter' | 'ferdig' | 'timeout'>('bruker')
  const [nyVersjon, setNyVersjon] = useState('')
  ...
  // Følg boksen gjennom restarten: vent til den går ned og kjem opp att,
  // og last så sida på nytt (for nye frontend-filer).
  const ventPaaRestart = async (versjon?: string) => {
    setNyVersjon(versjon || '')
    setRestartar(true)
    setRestartFase('bruker')
    let saaNede = false
    for (let i = 0; i < 45; i++) {
      ...
      try {
        ...
        if (!saaNede) {
          saaNede = true
          setRestartFase('venter')
        }
      }
    }
    setRestartFase('timeout')
  }
  ...
  const handleOppdaterFloate = async () => {
    ...
    try {
      ...
      setFloate(res)
      ventPaaRestart(res.hub?.versjon)
    } catch (e) {
      setMelding({ text: String(e), ok: false })
    }
    setBusy(false)
  ...
  if (loading) {
    return (
      <div className="panel mb-4 flex justify-center items-center h-32">
  ...
  if (restartar) {
    const ferdig = restartFase === 'ferdig'
    const timeout = restartFase === 'timeout'
    const fasetekst =
      restartFase === 'bruker' ? t('Update applied — restarting the box...')
      : restartFase === 'venter' ? t('Waiting for the box to come back...')
      ...
      <div className="panel mb-3">
        ...
        <div className="flex items-center gap-3 py-4">
          ...
          <span className="text-sm text-gray-700">
            {fasetekst}{nyVersjon ? ` (${nyVersjon})` : ''}
          </span>
        </div>
        {timeout && (
          <button
            onClick={() => window.location.reload()}
            className="bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium py-1.5 px-3 rounded-lg text-xs transition-colors"
          >
            {t('Reload now')}
          </button>
        )}
