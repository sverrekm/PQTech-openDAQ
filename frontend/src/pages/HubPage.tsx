      ...
      {hubAktiv && hub?.nodar && hub.nodar.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-4">
          ...
          <div className="space-y-2">
            {hub.nodar.map(n => {
              const erOpenDaq = !n.type || n.type === 'opendaq'
              return (
                <div key={n.id} className="flex items-center justify-between gap-3 px-3 py-2 bg-gray-50 rounded-md">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className={`w-2 h-2 rounded-full flex-shrink-0 ${n.tilkobla ? 'bg-green-500' : 'bg-gray-400'}`} />
                    <div className="min-w-0">
                      <div className="text-sm font-medium text-gray-900 truncate">{n.namn || n.id}</div>
                      <div className="text-xs text-gray-500 truncate">{n.adresse}{n.port ? `:${n.port}` : ''}{n.lokasjon ? ` · ${n.lokasjon}` : ''}</div>
                    </div>
                  </div>
                  {erOpenDaq && (
                    <a
                      href={`/node-proxy/${n.id}/`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex-shrink-0 text-xs font-medium px-3 py-1.5 rounded-md border border-gray-300 text-gray-700 hover:bg-white transition-colors"
                    >
                      {t('Open UI')} ↗
                    </a>
                  )}
